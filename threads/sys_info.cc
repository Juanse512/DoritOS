...
void SysInfo()
{
    (void) COPYRIGHT;  // Prevent warning about unused variable.
      ...
      unsigned numPhysicalPages = DEFAULT_NUM_PHYS_PAGES;

      #ifdef USER_PROGRAM
            extern Machine* machine;
            numPhysicalPages = machine->GetNumPhysicalPages();
      #endif
...
Memory:\n\
  ...
  Memory size: %u bytes.\n",
      PAGE_SIZE, numPhysicalPages, 
      TLB_SIZE, numPhysicalPages*PAGE_SIZE);
    printf("\n\
Disk:\n\
  Sector size: %u bytes.\n\
  Sectors per track: %u.\n\
  ...
  Disk size: %lu bytes.\n",
      ...
      NUM_TRACKS, NUM_TRACKS*SECTORS_PER_TRACK,
      sizeof(int) + NUM_SECTORS * SECTOR_SIZE);
    printf("\n\
Filesystem:\n\
